...
def extract_wt_sequence(mutant, mutated_sequence):
        ...
        # Check if the mutated amino acid matches the amino acid at the specified position
        # if mutated_sequence[position - 1] != mutated_aa:
        #     raise ValueError(f"The mutated amino acid {mutated_aa} at position {position} does not match "
        #                      f"the amino acid {mutated_sequence[position - 1]} in the mutated sequence.")
...
def blossum_score(mutant, blossum_dict):
    ref_aa = mutant[0]  # Extract reference amino acid from mutant, e.g., "A" from "A101C"
    mutant_aa = mutant[-1]  # Extract alt amino acid
    return blossum_dict[ref_aa + mutant_aa]
...
def add_cols(csv_file):
    '''
    :param df: pd df with 'mutant' 'mutated_sequence' columns
    :return: df with added column 'wt_sequence'
    '''
    
    df = pd.read_csv(csv_file)
    # Apply the function to create a new column for WT sequence
    df['wt_sequence'] = df.apply(lambda row: extract_wt_sequence(row['mutant'], row['mutated_sequence']), axis=1)
    # Apply the function to create a new column for WT sequence
    df['blossum'] = df.apply(lambda row: blossum_score(row['mutant'], blossum_dict), axis=1)
